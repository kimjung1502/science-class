...
META = {
    ...
    '통합과학2':      ('통합과학 2',      '통과2', 'public',     'blue',    '변화와 다양성, 환경과 에너지, 과학과 미래 사회를 탐구합니다.'),
    '과학탐구실험1':  ('과학탐구실험 1',  '과탐1', 'experiment', 'emerald', '과학사 속 탐구를 직접 체험하며 과학의 본성과 탐구 과정을 익힙니다.'),
    '과학탐구실험2':  ('과학탐구실험 2',  '과탐2', 'experiment', 'emerald', '생활 속 과학과 첨단 과학을 직접 실험하고 탐구합니다.'),
    '화학':           ('화학',            '화학',  'labs',       'orange',  '화학의 언어부터 물질의 구조, 화학 평형과 역동적 반응까지 탐구합니다.'),
    '물질과 에너지':  ('물질과 에너지',   '물질',  'bolt',       'violet',  '물질의 상태와 용액, 화학 변화의 자발성과 반응 속도를 다룹니다.'),
...
for ws in wb.worksheets:
    ...
    for raw in rows[1:]:
        ...
        if c_subj:
            name, short, icon, accent, desc = META[c_subj]
            s_ord += 1
            cur_subj = uuid.uuid4().hex
            subjects.append((cur_subj, name, short, desc, icon, accent, s_ord))
...
s_name = {i: n for (i, n, sh, d, ic, ac, o) in subjects}
...
lines.append('insert into subjects (name,short_name,description,icon,accent,sort_order,is_active) values')
lines.append(',\n'.join(
    f'({q(n)},{q(sh)},{q(d)},{q(ic)},{q(ac)},{o},true)' for (i,n,sh,d,ic,ac,o) in subjects) + ';')
